				...
				for (int c = 0; c < modifiedClusters.Count; c ++)
				{
					...
					{				
						...
						foreach (var _position in tileMap.clusters[modifiedClusters[c]].Keys)
						{
							...
							_tileObject = MonoBehaviour.Instantiate(prefab,  _tileData.position, _tileData.rotation);
							...
							//_clusterObject.transform.SetParent(_twc.worldObject.transform, false);
						//}
						}
						...
						_clusterObject.layer = assignLayer;
